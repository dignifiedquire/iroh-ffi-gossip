...
from iroh import IrohBuilder, MessageType, NodeOptions
from iroh_gossip import Gossip, GossipMessageCallback, alpn
...
async def test_gossip_basic():
    ...
    n0_builder = await IrohBuilder.create(NodeOptions())
    gossip0 = await Gossip.create(n0_builder.endpoint())
    n0_builder.accept(alpn(), gossip0)
    n0 = await n0_builder.build()

    n1_builder = await IrohBuilder.create(NodeOptions())
    gossip1 = await Gossip.create(n1_builder.endpoint())
    n1_builder.accept(alpn(), gossip1)
    n1 = await n1_builder.build()
