...
use iroh_ffi::{Endpoint, ProtocolHandler};

use crate::{CallbackError, GossipError};
...
impl ProtocolHandler for Gossip {
    async fn accept(
        &self,
        arc_conn: Arc<iroh_ffi::Connecting>,
    ) -> Result<(), iroh_ffi::CallbackError> {
        let conn = Arc::try_unwrap(arc_conn)
            .ok()
            .expect("should not be cloned");
        let conn = conn.into_iroh().await.expect("must not be reused");
        iroh::protocol::ProtocolHandler::accept(&self.gossip, conn).await?;
        Ok(())
    }

    async fn shutdown(&self) {
        iroh::protocol::ProtocolHandler::shutdown(&self.gossip).await;
    }
}
...
impl Gossip {
    ...
    pub async fn create(ep: Arc<Endpoint>) -> Result<Arc<Gossip>, GossipError> {
        let gossip = iroh_gossip::net::Gossip::builder()
            .spawn(ep.as_ref().into())
            .await?;

        Ok(Arc::new(Gossip { gossip }))
    }
...
impl Sender {
    ...
    #[uniffi::method(async_runtime = "tokio")]
    pub async fn broadcast(&self, msg: Vec<u8>) -> Result<(), GossipError> {
        self.sink
            .lock()
            .await
    ...
    #[uniffi::method(async_runtime = "tokio")]
    pub async fn broadcast_neighbors(&self, msg: Vec<u8>) -> Result<(), GossipError> {
        self.sink
            .lock()
            .await
    ...
    pub async fn cancel(&self) -> Result<(), GossipError> {
        if self.cancel.is_cancelled() {
            return Err(GossipError::from(anyhow::anyhow!("already closed")));
        }
...
#[cfg(test)]
mod tests {
    use iroh_ffi::{IrohBuilder, NodeOptions};
    use iroh_gossip::ALPN;
    use tokio::sync::mpsc;
    ...
    async fn test_gossip_basic() {
        let n0_builder = IrohBuilder::create(NodeOptions::default()).await.unwrap();
        let gossip0 = Gossip::create(Arc::new(n0_builder.endpoint()))
            .await
            .unwrap();
        n0_builder.accept(ALPN, gossip0.clone());
        let n0 = n0_builder.build().await.unwrap();

        let n1_builder = IrohBuilder::create(NodeOptions::default()).await.unwrap();
        let gossip1 = Gossip::create(Arc::new(n1_builder.endpoint()))
            .await
            .unwrap();
        n1_builder.accept(ALPN, gossip1.clone());
        let n1 = n1_builder.build().await.unwrap();
